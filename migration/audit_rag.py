...
==============================================================================
  ...
  USAGE:
      py -3.11 migration/audit_rag.py --agent buffett --query "stock based compensation" --company MongoDB
      py -3.11 migration/audit_rag.py --agent munger --query "moat durability"
...
def main_audit():
    ...
    if main.load_collection(mongo_philosophy_collection(agent)) is None:
        ...
        print(f"  Run: py -3.11 migration/ingest_philosophy.py --agent {agent}\n")
        sys.exit(1)
    ...
    if company:
        ...
        if company not in companies:
            ...
            print(f"  Ingest it first: py -3.11 migration/analyse_company.py --ticker <TICKER>\n")
            sys.exit(1)
